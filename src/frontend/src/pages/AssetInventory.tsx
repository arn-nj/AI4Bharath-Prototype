...
import { getAssets, assessAsset, assessBulk, fetchAllUnassessed, type AssetOut } from '../services/api';
...
export default function AssetInventory() {
  ...
  const [totalUnassessed, setTotalUnassessed] = useState<number | null>(null);
  ...
  // Don't reload mid-bulk — it would clobber the assets list used for filtering
  useEffect(() => { if (!bulkActiveRef.current) load(); }, [page, dept]);
  ...
  return (
    <div className="p-6 space-y-4">
      <div className="flex items-start justify-between gap-4">
        ...
        {/* Bulk assess button — counts ALL pages, runs in parallel */}
        <button
          onClick={async () => {
            ...
            try {
              // Fetch all unassessed across every page first
              const all = await fetchAllUnassessed();
              if (all.length === 0) { return; }
              setTotalUnassessed(all.length);
              setBulkProgress({ done: 0, total: all.length });
              await assessBulk(
                all.map(a => a.asset_id),
              ...
              setTotalUnassessed(null);
            }
          }}
          disabled={bulkAssessing}
          className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold bg-green-600 text-white hover:bg-green-700 disabled:opacity-60 transition-colors shadow-sm flex-shrink-0"
        >
          {bulkAssessing ? (
            <>
            ...
            <>
              ▶▶ Assess All Unassessed
              {totalUnassessed != null && (
                <span className="bg-green-500 text-white text-xs rounded-full px-1.5 py-0.5">{totalUnassessed}</span>
              )}
            </>
          )}
        </button>
